...
default_config = PlotConfig(
    ...
    25, 
    joinpath(@__DIR__, "1223.txt"),                           #file_path
    joinpath(@__DIR__,"basic.txt") ,                          #bascic_path
    [:purple,Images.RGB(0/255, 0/255,90/255),:darkblue,Images.RGB(0/255, 0/255,150/255) ,Images.RGB(0/255,100/255,0/255),:darkgreen], # colors
    joinpath(@__DIR__,"0104.png"),                                     #result
...
function draw2pic(cfg::PlotConfig,title::String,result::String)
    # 读取第一个文件，将问题出现次数存储在字典1中
    problem_counts1 = Dict{String, Int}()
    open(cfg.file_path) do file
        for line in eachline(file)
    ...
    colors=cfg.colors[1:length(frequencies)]
    colors_length=length(cfg.colors)
    ax = Axis(fig[1, 1],xticks=(1:colors_length,label_vector ),yticks=0:value,title=title,titlesize=cfg.title_size,xticklabelrotation = π/18)
    barplot!(fig[1,1],values_vector,color=colors )
    fig
    CairoMakie.save(result, fig)
