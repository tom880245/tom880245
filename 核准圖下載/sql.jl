...
default_config = Config(
    ...
    "https://building.e-land.gov.tw/upload/getDocfileAction.do?index_key=",
    "http://build.kinmen.gov.tw/upload/BDMfileok.jsp?index_key=",
    "https://building.e-land.gov.tw"
    
    )

#https://building.e-land.gov.tw/upload/finishpdf/S7.PDF?index_key=1130403142107&last_modify=01059&fn=S7004.pdf&vid=
# try
#     using HTTP
# catch
...
function Truepic(data::String)
    ...
    h = parsehtml(String(r.body))
    newanime_item = map(x -> x.attributes["href"],eachmatch(Selector("a"), h.root))
...
function GCGBM(index_key::String)
    #先寫讀取turepic 
    local Classification=String[]


    r = HTTP.get("$(default_config.approve)$(index_key)",headers=default_config.headers)
    h = parsehtml(String(r.body))
    newanime_item=map(x->x.attributes["href"],eachmatch(Selector("a"),h.root))
    filename = joinpath(@__DIR__,"output.txt")
    open(filename, "w") do file
        for i in newanime_item
            GCGBM_url="start "*default_config.GCGBM_approve*i
            write(file, GCGBM_url*"\n")
        end
    end
